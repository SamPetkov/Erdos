...
\paragraph{Purpose of this section.}
Section~7 controls the diagonal contribution coming from common whole
classes. We now decompose every remaining overlap canonically according to
its high cells and sum the resulting high-cell contribution, including
unequal-type and nonendpoint cells. The local and cycle factors of the
unexposed residual configuration are deliberately deferred: Section~9 will
bound them uniformly after the present bare high-skeleton sum has been
established.
...
We first sum the bare high-skeleton contribution, postponing all local and
...
\paragraph{Exact endpoint bookkeeping.}
...
\paragraph{Near-containment decorations.}
...
\paragraph{Middle-strip residual factor.}
...
\[
 ...
 =\exp\{o(n/(\ln n)^4)\}.\qquad\square                       \tag{8.30}
\]
...
\paragraph{Purpose of this section.}
Fixing a canonical high skeleton does not yet control the local multiplicity
factors or the cycle-space factor of the residual overlap graph. The
conditional quantity defined below collects precisely these residual
contributions, subject to the residual cap and the prohibition on adding a
further pair in a high cell. We prove a bound uniform over all feasible
skeletons; Proposition~9.2 then combines that bound with the bare
high-skeleton estimate from Section~8.

\paragraph{Exact conditional decomposition.}
...
\[
 \sum_{(\mathcal M,j)}\pi(\mathcal M,j)\left(\prod_{e\in \mathcal M}g(j_e)\right)
...
\paragraph{Proof: large residual degree.}\label{proof-large-residual-degree}
...
After fixing an even subgraph and expanding these alternatives, apply
Lemma~6.2 jointly to all demanded cells. Every coefficient and event
indicator is nonnegative, so the residual cap may be removed only after this
joint bound is taken; doing so enlarges the event and preserves the stated
upper-bound direction. This gives
...
row--column cells), and \(\mathcal C_{\rm even}(\mathcal M,R)\) is the finite family of
edge subsets \(F\subseteq E(\mathcal M\cup R)\) having even degree at every vertex;
the edges of \(\mathcal M\) have weight one in the displayed product.
...
\paragraph{Cycle-kernel estimate.}
...
weights are nonnegative, this relaxation can only increase the sum. If
\(w(C)=\prod_{e\in C\setminus \mathcal M}q_e\), then, with
\(w(F):=\prod_{e\in F\setminus \mathcal M}q_e\),
...
\paragraph{Proof: small residual degree.}\label{proof-small-residual-degree}
...
\paragraph{Proof.}\label{proof-8}

Equation (9.2) is an exact canonical decomposition.  Write
\[
 w(\mathcal M,j):=\pi(\mathcal M,j)\prod_{e\in \mathcal M}g(j_e).
\]
Lemma 8.3 sums all
...
simultaneous leftover coloring that controls the added parts.
...
also available in Markdown, so the argument can be inspected without
compiling the TeX. The dossier includes the TeX source, bibliography,
reproducible build material, audit records, and the current partial Lean~4
formalisation.
...
ChatGPT-5.6 Sol Pro and Aristotle were consulted for proof development,
adversarial checking, and the Lean formalization. In particular,
ChatGPT-5.6 Sol Pro proposed the four-size signed profile with class sizes
