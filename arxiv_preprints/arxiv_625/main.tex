...
The proof is organized around the rounding phase of the
independence-number threshold. We compare ordinary colorings with signed
cocolorings whose classes have four consecutive sizes and may be declared
either independent or complete. This extra choice lowers the first-moment
root by order \(n/(\ln n)^3\), uniformly throughout the phase. To show that
the lower root is attained, we derive an exact overlap identity and express
its sign contribution through even subgraphs of a bipartite configuration
model. The resulting second-moment estimate produces a possibly rare
cocoloring, which a vertex-exposure argument promotes to a high-probability
event without changing the leading root separation.
...
Sections~2--4 establish an unrestricted lower location for \(\chi(G_n)\).
Section~5 constructs a
four-size signed cocoloring profile whose first moment is positive at a
threshold lower by order \(n/(\ln n)^3\). Sections~6--9 establish the
corresponding normalized second-moment estimate: Section~6 gives the exact
overlap and sign decomposition, Section~7 controls common whole classes,
Section~8 sums the high-cell skeletons, and Section~9 bounds the
remaining conditional local and cycle contributions. Section~10 amplifies the
resulting possibly rare cocoloring event to a high-probability statement.
Finally, Section~11 combines the chromatic lower location with the
cocoloring upper location. The roadmap records dependencies only; each
...
Applying (1.2) to the factorials in its
exact signed first moment
...
A partial Lean~4 formalization developed using ChatGPT-5.6 and Aristotle
\citep{achim-et-al-2025} is available in a self-contained source file on
\href{https://github.com/SamPetkov/Erdos/blob/main/625/formalization/Erdos625SelfContained.lean}{GitHub}. Each statemet was broken down into substatements and verified on input and output for a safer formalisation.
