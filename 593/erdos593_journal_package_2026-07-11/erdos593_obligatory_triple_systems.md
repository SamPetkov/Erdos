...
subtitle: "A self-contained proof, with comparison to arXiv:2606.24882"
author: "Journal-style working manuscript"
date: "11 July 2026"
bibliography: references.bib
---

# Scope, provenance, and comparison
...
This is a revised, self-contained presentation of the proof developed in this conversation. It is not a transcription of Eric Li's June 2026 preprint [@li2026]. Comparison with that paper prompted three explicit repairs or expansions: the reduction of membership in the constructive class under deletion of isolated vertices is proved by structural induction; the rooted-copy auxiliary graph is formed on the correct vertex set; and the bridge-block reconstruction includes a full running-intersection argument.

## How arXiv:2606.24882 helps, and how the proofs differ

Li's preprint states the same classification and proves a stronger exact bridge-trace theorem for a complete-rank one-apex sequence lift [@li2026]. It is therefore a useful external audit of the proof architecture. The two approaches share the following structural spine:

1. a one-apex sequence lift whose base is an edge of an auxiliary graph;
2. a transfinite branch argument preserving uncountable chromatic number;
3. finite trace control forcing a Levi bridge at each hyperedge and collapsing Berge cycles to ordinary graph cycles;
4. a bridge quotient forest and reconstruction by one-point amalgamations.

The proofs are not identical in detail.

- **Positive atom.** The proof below directly proves that $K_{n,n}^{+}$ is obligatory, using a bounded-codegree decomposition, finite-closure chains, and a rainbow bipartite submatrix lemma. Li imports Reiher's stronger uniform expansion theorem [@reiher2024].
- **One-point closure.** The proof below uses a rooted-abundance lemma that yields many pairwise disjoint petals at one root. Li selects one rooted copy at each eligible root and applies a bounded-outdegree auxiliary graph.
- **Finite decomposition.** The proof below deletes all Levi bridges and works with bridge-free components. Li selects one bridge at every hyperedge-node and uses graph derivatives.
- **Trace theorem.** The factor-by-base-node theorem below gives the necessity required for avoidance. Li proves an if-and-only-if bridge-selector theorem, which is stronger.
- **Avoidance inputs.** For nonlinearity, the proof below uses an explicit Erdős--Rado triangle system. For odd Berge cycles, it proves directly that suitable shift graphs have uncountable chromatic number and arbitrarily large odd girth. Li imports the classical nonlinearity and high-odd-girth existence interfaces.
- **Scope.** Li also proves exact-cardinal calibration and resolves Erdős Problem 1177. This manuscript treats Problem 593 only.

> **Status note.** The preprint independently confirms the theorem and most of the global architecture. It does not verify every line of the alternative arguments below. The direct expansion atom and the explicit shift-graph odd-girth construction are the principal proof components specific to this presentation. No blocking discrepancy was found in the comparison, but this is an internal mathematical audit, not formal verification or peer review.

## Dependency audit

Only two non-elementary external results are used as black boxes in the proof below:

- the de Bruijn--Erdős compactness theorem for finite graph colourings [@debruijn1951];
- the pair case of the Erdős--Rado partition theorem [@erdosrado1956], namely
  $$(2^{\aleph_0})^+\longrightarrow(\aleph_1)^2_{\aleph_0}.$$

The obligatory expansion atom, rooted closure, high-odd-girth auxiliary graphs, sequence-lift trace theorem, and Levi-block reconstruction are proved in full. The papers of Erdős--Hajnal--Rothschild, Komjáth, Hajnal--Komjáth, Reiher, and Li supply historical and comparative context [@ehr1973; @komjath2001; @hajnalkomjath2008; @reiher2024; @li2026]. The original problem is recorded in Erdős's 1995 paper and in the current Erdős Problems entry [@erdos1995; @bloom593].
...
For the second, every finite subgraph has at most $d|V|$ edges and therefore a vertex of degree at most $2d$. It is $(2d+1)$-colourable by greedy deletion. The de Bruijn--Erdős compactness theorem then gives a $(2d+1)$-colouring of all of $G$. $\square$
...
**Proof.** We use the standard Erdős--Rado relation
...
The following table records every source included in the accompanying bibliography and the precise role, if any, it plays in this manuscript.

| Source | Exact role in this manuscript | Dependency status |
|---|---|---|
| de Bruijn--Erdős (1951) [@debruijn1951] | Compactness: if every finite subgraph is $k$-colourable, then the full graph is $k$-colourable. | Black box used in Lemma 1.3. |
| Erdős--Rado (1956) [@erdosrado1956] | The partition relation $(2^{\aleph_0})^+\to(\aleph_1)^2_{\aleph_0}$. | Black box used in Proposition 8.1. |
| Erdős (1995) [@erdos1995] | Historical source for Problem 593. | Context only. |
| Erdős--Hajnal (1966) [@erdoshajnal1966] | Classical high-chromatic graph and set-system background. | Context; not invoked in the proof. |
| Erdős--Hajnal--Rothschild (1973) [@ehr1973] | Classical nonlinearity obstruction used by Li's paper. | Comparative context; replaced here by Proposition 8.1. |
| Erdős--Galvin--Hajnal (1975) [@egh1975] | Large-chromatic set-system background and an interface used in Li's exact-cardinal part. | Context only. |
| Komjáth (2001) [@komjath2001] | Earlier block reduction and tripartiteness results for obligatory triple systems. | Historical context. |
| Hajnal--Komjáth (2008) [@hajnalkomjath2008] | Results on linearly obligatory triple systems. | Context only; not needed for Problem 593 here. |
| Komjáth (2008) [@komjath2008] | A consistent construction avoiding selected configurations. | Context only. |
| Reiher (2024/Proc. AMS) [@reiher2024] | Stronger theorem that all uniform private expansions of $K_{n,n}$ are obligatory. | External corroboration; Proposition 3.2 supplies an independent $3$-uniform proof. |
| Reiher, *Graphs of large girth* [@reihergirth2024] | Modern high-girth reference and the high-odd-girth interface used by Li. | Context; replaced here by Section 7. |
| Li (2026) [@li2026] | Same classification, stronger exact bridge-trace theorem, and resolution of Problem 1177. | Comparative audit, not a premise. |
| Wang--Duan--Gerbner--Hama Karim (2026) [@wang2026] | Finite weak-colouring context for Berge-cycle-free forbidden hypergraphs. | Context only. |
| Erdős Problems, Problem 593 [@bloom593] | Current problem statement and bibliography. | Specification/status source. |

# Audit checklist

The proof explicitly addresses the main failure modes in the problem specification:

- the host chromatic number is strictly greater than $\aleph_0$ in every positive and negative construction;
- embeddings are non-induced throughout;
- finite deletion is used to force disjoint copies;
- one-point closure is proved with a rooted-copy argument rather than assumed;
- isolated vertices are treated for both obligatoriness and membership in $\mathcal B$;
- a Berge cycle of length $m$ is distinguished from its Levi cycle of length $2m$;
- the bridge condition is checked at every hyperedge-node, not merely componentwise;
- no incidence bridge is assumed to arise from a globally private vertex;
- the quotient forest is supplemented by a running-intersection proof;
- every avoidance host is genuinely $3$-uniform, simple, uncountably chromatic, and $F$-free under non-induced embeddings.
