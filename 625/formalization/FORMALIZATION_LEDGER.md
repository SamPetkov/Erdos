...
| `MatchingExtensions`, `card_extensions_of_exposed_equiv` | fixed-exposure count before (6.8), and residual count after (8.3) | defined; exact cardinal identity proved | Full bijections extending an exposed finite pairing are explicitly equivalent to arbitrary bijections between the complements. Hence, when the two ambient finite types have equal cardinality, their exact number is `(card L-card S)!`. The global configuration-model stub encoding, witness-to-exposure map, event coverage, uniform-law transport, and (6.8) union bound remain open. |
