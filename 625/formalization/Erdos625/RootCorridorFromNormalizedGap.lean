...
the signed root gives eventual positivity. Second, the uniform positive phase
margin forces a fixed positive normalized root gap, while the natural root-gap
scale tends to infinity; hence the actual gap eventually exceeds two.
