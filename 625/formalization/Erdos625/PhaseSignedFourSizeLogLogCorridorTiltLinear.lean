...
/-!
# Tilt-linear control on the logarithmic-logarithmic root corridor

Uniform compact control bounds the finite selected tilt throughout the
enlarged root-search corridor.  The exact target identity then turns the
remaining `n / s` factor into a quantity no larger than the phase parameter.
-/
...
the absolute tilt-linear term in the exact signed four-size derivative is
eventually at most one thirty-second of the quadratic main scale. -/
theorem eventually_abs_phaseRootLogLogCorridor_fourSize_tilt_linear_le_quadratic
    (C : Real) (hC : 0 ≤ C) :
    ∀ᶠ n : Nat in atTop,
      ∀ s ∈ Icc
          (phaseRootCenter n -
...
#print axioms eventually_abs_phaseRootLogLogCorridor_fourSize_tilt_linear_le_quadratic
