...
cells.  The total space of such decorated supports is tautologically equivalent
to the dependent sum over endpoint tables of the already defined
`FourEndpointDecoratedBlockPairing` fibres.

Consequently the total reference sum is exactly `sum_L fourEndpointW(L)`.  This
module is finite and exact: no endpoint transport inequality or asymptotic
...
/-- The finite set of endpoint tables actually realized by an abstract block
support.  `FourEndpointFullTable` itself is infinite because its entries are
unrestricted naturals, so every finite sum must use this realized subtype. -/
noncomputable def fourEndpointRealizedFullTables
    (alpha : Nat) (hAlpha : 5 < alpha)
    (k : ColoringProfile (alpha + 1)) : Finset FourEndpointFullTable := by
  classical
  exact
    (Finset.univ :
      Finset (FourEndpointAbstractBlockSkeleton alpha hAlpha k)).image
        (fourEndpointSupportTable alpha hAlpha (k := k))

/-- The total decorated-support space is exactly the dependent sum of the
fibres over realized endpoint tables. -/
def fourEndpointAllDecoratedSupportEquivSigmaTable
  ...
  toFun z :=
    ⟨⟨fourEndpointSupportTable alpha hAlpha z.1, by
        classical
        simp [fourEndpointRealizedFullTables]⟩,
      ⟨fourEndpointBlockPairingOfSupport alpha hAlpha z.1, z.2⟩⟩
  invFun z := ⟨z.2.1.1, z.2.2⟩
  left_inv z := rfl
  right_inv := by
    rintro ⟨⟨L, hLmem⟩, ⟨P, hP⟩, decoration⟩
    have hL : fourEndpointSupportTable alpha hAlpha P = L := by
...
over all block supports gives exactly the existing endpoint-table sum. -/
theorem sum_fourEndpointFullSupportReferenceWeight_eq_sum_W
    (n alpha : Nat) (hAlpha : 5 < alpha)
    (k : ColoringProfile (alpha + 1)) :
    (∑ P : FourEndpointAbstractBlockSkeleton alpha hAlpha k,
...
#print axioms sum_fourEndpointFullSupportReferenceWeight_eq_sum_W
