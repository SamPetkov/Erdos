...
existing endpoint-table fibres. -/
def fourEndpointAllDecoratedSupportEquivSigmaTable
    (alpha : Nat) (hAlpha : 5 < alpha)
    (k : ColoringProfile (alpha + 1)) :
    FourEndpointAllDecoratedSupport alpha hAlpha k ≃
      Σ L : FourEndpointFullTable,
        FourEndpointDecoratedBlockPairing alpha hAlpha k L where
  toFun z :=
    ...
    (∑ P : FourEndpointAbstractBlockSkeleton alpha hAlpha k,
      ...
      ∑ z : FourEndpointAllDecoratedSupport alpha hAlpha k,
        fourEndpointFullSupportAtomWeight n alpha hAlpha z.1 := by
          ...
          rfl
    _ = ∑ z : Σ L : FourEndpointFullTable,
          FourEndpointDecoratedBlockPairing alpha hAlpha k L,
        fourEndpointDecoratedReferenceAtomWeight n alpha hAlpha z.1 := by
          simpa [equivalence, fourEndpointFullSupportAtomWeight,
            ...
            equivalence.sum_comp
              (fun z : Σ L : FourEndpointFullTable,
                FourEndpointDecoratedBlockPairing alpha hAlpha k L =>
