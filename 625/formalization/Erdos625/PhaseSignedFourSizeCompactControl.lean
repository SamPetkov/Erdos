...
/-!
# Uniform finite-four control on the native phase-root corridor

This is the compact-target bridge required before the signed derivative
estimate.  It specializes the existing four-score uniformity APIs to the
moving phase parameter and the native gap corridor.  The result supplies both
uniform convergence of the selected tilt and a common positive lower bound on
all four finite optimizer coordinates.
-/
...
/-- On the native phase-root gap corridor, the finite four-deficit tilt is
eventually uniformly close to the limiting Gaussian tilt, and every finite
optimizer coordinate has one common positive lower bound. -/
theorem exists_pos_eventually_phaseRootGapCorridor_fourSize_compact_control :
    ∃ c > 0, ∀ ε > 0,
      ∀ᶠ n : Nat in atTop,
        ∀ s ∈ Icc
