...
/-- The scalar term at the current reference center diverges to `+∞`.
This signed conclusion is stronger than the public `Θ` statement and is the
quantity used to test a bounded center-value margin. -/
theorem tendsto_phaseRootScalarTerm_atTop :
    Tendsto phaseRootScalarTerm atTop atTop := by
  have hc : 0 < 2 / q - 1 / 2 := cLead_pos
  have hb := phaseRootScalarTerm_sub_lead_isLittleO.bound
    (by linarith : (0 : ℝ) < (2 / q - 1 / 2) / 2)
  ...
  intro B
  filter_upwards
    [hb, tendsto_logLogOrder_atTop.eventually_ge_atTop
      (max 0 (2 * B / (2 / q - 1 / 2)))] with n hbound hlog
  have hlogNonneg : 0 ≤ logLogOrder n :=
    (le_max_left 0 (2 * B / (2 / q - 1 / 2))).trans hlog
  have hscaled : B ≤ (2 / q - 1 / 2) / 2 * logLogOrder n := by
    have hdiv : 2 * B / (2 / q - 1 / 2) ≤ logLogOrder n :=
      (le_max_right 0 (2 * B / (2 / q - 1 / 2))).trans hlog
    have hmul := (div_le_iff₀ hc).mp hdiv
    nlinarith
  rw [Real.norm_eq_abs, Real.norm_eq_abs, abs_of_nonneg hlogNonneg,
    abs_le] at hbound
  nlinarith [hbound.1]
