...
**Audit branch:** `agent/625-line-by-line-theorem-audit`  
...
## 5. Lemma 2.1: lines 295--437 — **GREEN**
...
## 6. Lemma 3.1: lines 495--740 — **GREEN-REWRITE**
...
- Lines 764--80 sum all bounded profiles. There are
  \(\exp(O((\log n)^2))\) such profiles, and zero profile coordinates are
  handled by the zero-safe Stirling bound.
- Lines 82--95 use the derivative from Lemma 3.1 to move
...
## 8. Section 5 and Lemma 5.1: lines 798--1307 — **GREEN-REWRITE**

### Lemma 5.1 statement: lines 860--869
...
### Root displacement and midpoint profile: lines 1123--1307
...
The text loses more constant than required:

1. (5.11) gives the phase coefficient \((\log2)^2/4\);
2. midpoint placement retains one half, giving \((\log2)^2/8\);
3. integer rounding and amplification are lower order and do not require
   another fixed halving.

After Section VIII closure, the natural theorem is
...
## 9. Lemma 6.1: lines 1354--1428 — **GREEN**

### Statement

The normalized sign factor is
...
## 10. Lemma 6.2: lines 1445--1523 — **GREEN**
...
## 11. Section 7 and Lemma 7.1: lines 1525--1935 — **GREEN-REWRITE**

### Exact identities: lines 1525--1649
...
- **Source defect:** equation (7.2) writes `2^\ell_\bullet`. It must be
  `2^{\ell_\bullet}`. This is TYPO/EDITORIAL.
- Equations (7.3)--(7.6) follow by exact factorial cancellation and one-coordinate
  ratios. They are mathematically correct.

### Lemma 7.1 statement: lines 1651--1659

The target \(\sum_\ell D(\ell)=1+o(1)\) is exactly what endpoint transportation
needs.

### Empty corner: lines 1667--1704

- The lower bound on \(\mu_{u_i}(n)\) gives
  \(\Xi_{\mathrm{empty}}=o(1)\).
- If selected mass is at most \(\eta n\), the changing ambient vertex count costs
  at most a small power of \(\log n\).
- Iterating (7.4) in any order gives a product of exponential-series terms, and
  the total is \(1+o(1)\).
...
- Lines 1848--1878 prove uniform negativity by two convex endpoint checks.
  The decimal inequalities appear correct, but they should be replaced by the
  exact rational certificate from the later audit stack.
- Lines 1879--1906 show that the negative leading term dominates entropy and
  logarithmic errors uniformly when the selected mass is away from both
  corners.

### Full corner: lines 1907--1935

- The shift estimates give \(\mu_{u_i}(n)\le n^{6+o(1)}\) uniformly over the four
  sizes.
- For residual mass at most \(n/32\), the factor \(32^{-u_i}\) supplies
  \(n^{-10+o(1)}\), making the reverse recurrence strictly decreasing.
- Division by the exponentially large complete signed first moment makes the
  entire full corner negligible.

**Repair:** fix (7.2), replace decimal central-rate checks with the exact
certificate, and bind every \(o(1)\) to a deterministic phase-uniform sequence.

## 12. Section 8 up to Lemma 8.2: lines 1937--2228

### Canonical high support: lines 1937--2052 — **GREEN-REWRITE**

- A cell above \(R_0=\lfloor u_{\max}/2\rfloor\) is unique in its row and column,
  so high cells form a matching.
- Selecting exact high-cell stub pairs gives (8.3).
- Conditional cancellation with the residual matching recovers the exact
  overlap law (6.2).
- The endpoint reference weights \(W(L)\) and one-sided diagonal weights \(D(r)\)
  have now been independently normalized in the formal stack.

The prose should distinguish clearly between:

1. a block-level matching support;
2. a physical partial stub matching in a selected cell;
3. an aggregate fibre sum.

### Lemma 8.1: lines 2053--2146 — **GREEN-REWRITE**

- The global falling-factorial comparison follows from concavity of
  \(f(x)=\log(n)_x\) and the exact mass identity (8.11).
- The local unequal-size factor follows from (8.13).
- The phase relation gives the stated \(\eta_n\).
...
### Lemma 8.2: lines 2151--2228 — **GREEN-REWRITE**

- Cauchy's inequality and the two nonnegative domain enlargements are valid.
- The multinomial theorem gives the one-sided row and column sums.
- The number of margin vectors contributes only a polynomial factor.
- Lemma 7.1 closes the diagonal sum.
...
directly, without \((\sum\sqrt D)^2\). Use that route in Version 2.

## 13. Lemma 8.3: lines 2233--2490 — **RED**

This is the only independent submission blocker.

### Statement: lines 2233--2243

The desired conclusion is exactly the necessary bare-skeleton estimate. The
problem is not the target but the finite identification used to reach it.

### Proof introduction: lines 2245--2263

The near/middle charging narrative announces an objectwise completion picture.
That picture must be interpreted only after summing physical fibres: a partial
physical matching generally has many full completions and may leave stubs that
participate in residual cells.

### Step I, lines 2264--2284

- Writing \(j=m-e\) is unique.
- The high/middle inequalities are arithmetically correct.
- Replacing selected multiplicities by full endpoint multiplicities produces a
  valid block-level reference table.
- What is missing is a weight-preserving finite theorem connecting the actual
  physical high-skeleton family to that reference table and deficit data.
...
- **Blocking transition:** equations (8.25a)--(8.26) and the sentence
  ...
  decomposition. This is the precise gap.

### Steps III--IV, lines 2370--2469

- The joint threshold estimate is applied before constraints are relaxed, so
  the upper-bound direction is preserved.
- The finite sums use ranges such as \(3a/4+O(1)\), which are not literal finite
  index sets and must be replaced by exact floor/ceiling bounds.
- The large-residual quadratic exponent is negative on the stated interval.
- The small-residual pointwise bound is conservative.
- Step IV pays residual local and cycle factors even though the Section VIII
  contract says those factors are deferred. This may still be an upper bound,
  because the inserted factors are at least one, but it obscures the
  factorization and creates apparent double charging.
...
For a block support \(P\), endpoint minima \(m_e\), and deficits \(h_e\), prove
an exact finite decomposition with aggregate weight

\[
 w(P,m-h)=
 \frac{\prod_e(s_e)_{m_e-h_e}(t_e)_{m_e-h_e}}
      {(n)_{\sum_e(m_e-h_e)}\prod_e(m_e-h_e)!}
 \prod_e g(m_e-h_e).
\]
...
\[
 ...
 \le
 \prod_e n^{h_e}R_{m_e,d_e}(h_e)
\]

uses the local ratio cell by cell and the global denominator only once.

The checked sharper arithmetic gives, whenever \(2h<m\),
...
\[
 n^hR_{m,d}(h)
 \le
 \left(\frac{nm}{2^{\lfloor(3m-1)/4\rfloor}}\right)^h,
\]

and the complete positive-deficit fibre is summed before supports are
multiplied. This yields the direct all-deficit exponent

\[
 O(\sqrt n(\log n)^{3/2}),
\]

or \(O((\log n)^4)\) with the optional first-term/tail split. Both are far below
\(n/(\log n)^4\).

## 14. Section 9: lines 2492--2865

### Exact decomposition (9.2): lines 2492--2548 — **GREEN-REWRITE**

The normalized second moment should indeed be an exact sum of bare high-skeleton
...
### Lemma 9.1: lines 2550--2827 — **SUPERSEDED**

The old proof has the following internally coherent stages:

- lines 2567--2663: expand local factors and derive cubic/quadratic activity
  bounds;
- lines 2664--2705: charge local increments and the even-subgraph factor without
  double charging one cell in a monomial;
- lines 2706--2734: bound total local activity and row/column sums;
- lines 2735--2798: decompose even sets into cycles and estimate residual-only
  and mixed cycles by walk kernels;
- lines 2801--2827: handle small residual mass directly.

No explicit counterexample to the stated upper bound was found. Nevertheless,
the proof should be replaced. Deleting the exposed matching is injective on the
family of even residual edge sets, so
...
Moreover, the local increment is pointwise dominated by the same \(q\).
Consequently both finite products are charged to one total-\(q\) sum. The
checked q-only two-regime theorem yields the literal attained attachment bound
without cycle decomposition, walk kernels, \(	au\), or \(h\tau\).

### Proposition 9.2: lines 2832--2865 — **RED**

- Equation (9.2) plus a uniform attachment bound and a bare-skeleton sum gives
  the displayed upper bound immediately.
- The normalized second moment is at least one by variance, so its logarithm is
  nonnegative.
- The proof is logically correct conditional on Lemma 8.3.

**Repair:** compose the future green bare-skeleton theorem with the existing
q-only second-moment frontier.

## 15. Lemma 10.1: lines 2903--2963 — **GREEN**

- A union bound makes every \(n^{1/4}\)-set dense in the complement.
- Averaging over \(n^{1/4}\)-subsets propagates the same density lower bound to
  every larger set.
- The maximum-degree neighborhood recurrence
  \(s_{t+1}\ge(s_t-1)/4\) is correct.
- Iterating from size \(n^{1/3}\) leaves at least \(n^{1/4}\) vertices for a
  positive constant times \(\log n\) steps.
- The chosen vertices form a complement clique and hence an independent set in
  \(G_n\).
- Repeated removal and singleton coloring of the remainder give (10.3)
  simultaneously for every set \(S\).

No gap was found.

## 16. Lemma 10.2: lines 2968--3046 — **GREEN**

- \(S_k\), the largest induced subset cocolorable with \(k_n\) parts, changes by
  at most one when one vertex-exposure block is changed: delete the affected
  vertex and the remaining graph is identical.
- The seed event is exactly \(S_k=n\).
- The upper one-sided McDiarmid tail turns seed probability \(e^{-\Lambda_n}\)
  into the expectation deficit (10.7).
- The lower tail gives the random leftover size (10.8).
- A maximizing induced set and an ordinary coloring of its complement give
  (10.9).
- The failure probability of the simultaneous Lemma 10.1 event is one
  deterministic \(arepsilon_n=o(1)\), independent of \(k_n,\Lambda_n,r\).

The tunable family (10.5) is a useful standalone corollary and should be named
in Version 2.

## 17. Lines 3047--3111: amplification application and final proof

### Lines 3047--3077 — **AMBER only through Proposition 9.2**

With \(B_n=n/(\log n)^4\) and \(r_n=\sqrt{B_n}\), all three added-part terms are
\(o(n/(\log n)^3)\) if
\(\Lambda_n=o(n/(\log n)^4)\). The calculations are correct.

### Lines 3078--3111 — **RED only through Proposition 9.2**

- Intersecting the chromatic lower event and cochromatic upper event requires no
  independence; the union bound is sufficient.
- Subtracting locations gives (11.1).
- The final fixed halving from (11.1) to (11.2) is conservative, not necessary.
- The full-sequence conclusion follows from phase uniformity once every
  dependency has been closed.

After closure, replace the printed /32 theorem by the phase-resolved /8 theorem
and its stronger fixed-constant corollary.

## 18. Exact defects and mandatory edits
...
| Theorem 1 lines 121--130 | Unconditional theorem | RED dependency | Keep conditional in working draft. |
...
| Multiple `o(1)` events | No named deterministic sequence | GREEN-REWRITE | Bind to explicit phase-uniform sequences in final theorem statements. |

## 19. Version 2 dependency graph
...
## 20. Acceptance gates before changing theorem status
...
4. `profileHighSkeletonWeight` is identified pointwise with that aggregate
   weight;
5. the one global falling-factorial comparison is applied after, not before,
   the aggregate fibre sum;
...
12. the TeX, bibliography, source-level typo, theorem coefficient, Lean status,
    and reproducibility language are synchronized.
...
coherent. The exact signed overlap identity and the amplification argument are
particularly clean.
...
“complete, distinguish, and forget” prose. The repair is already sharply
specified and supported by several green finite components, but the final
pointwise aggregate-weight identification and global bare-skeleton assembly
