...
\]
The zero-residual exponent is zero. Thus all three cases are
$o(n/(\log n)^4)$, uniformly in the high skeleton. Hence there is a
deterministic sequence $\eta_n\to0$ such that
\begin{equation}
  \mathcal A(M,j)
  \le
  \exp\!\left\{
...
remaining high-skeleton sum. Adding the two deterministic logarithmic error
sequences gives the required $\Lambda_n$. The lower bound follows from
nonnegativity of the variance.
\end{proof}
