...
box. More precisely, the lower bound $p_i\ge c_p>0$ from (5.15) implies that
there is an absolute $C_{\mathrm H}$ such that, for
$c_p\le p\le1$ and $0\le y\le p$,
\begin{equation}
...
by compactness. Summing (7.14a) over the four coordinates gives the required
$O(Y\log(\mathrm e/Y))$ bound.
...
\]
If $\rho\ge1/32$, then $R\ge1/64$ for all sufficiently large $n$. Hence
$x\mapsto x\log x$ is uniformly Lipschitz on the interval between $R$ and
$\rho$. Moreover, $-R\log R\le1-R=Y$. Thus
\[
\begin{aligned}
&\left|n\rho\log\rho
