...
\[
  ...
  \right\rceil.
\]
At this fixed value of $k_{\mathrm{co}}$, the tangent correction in Section~5
changes only the four type multiplicities by $O(1)$. It preserves exactly both
\[
...
\subsection{An exact four-support certificate with fixed slack}

It remains to replace the phase-dependent factor $A_4(\delta)$ by one explicit
constant and, crucially, to retain a fixed positive margin over the coefficient
stated in the theorem. Put $q=\log 2$, let
\[
...
\end{equation}
The strengthened certificate has an explicit fixed margin over the constant in
the theorem:
\begin{equation}
  \log\!\left(\frac{20000}{12777}\right)
  =
  ...
  \label{eq:fixed-four-support-slack-v3}
\end{equation}

Combining \eqref{eq:phase-resolved-final-v3} with
\eqref{eq:exact-four-support-certificate-v3}, and then taking $n$ large enough
that
\[
  \epsilon_n<\frac{(\log 2)^2}{8}\sigma_4,
\]
yields
\[
  \mathbb P\!\left(
...
\]
The stronger certificate before absorbing the asymptotic error has coefficient
\[
  \frac{(\log 2)^2}{8}
  \log\!\left(\frac{20000}{12777}\right)
  =0.026910509263013\ldots.
\]
...
$\zeta(\overline G)=\zeta(G)$. The preceding result gives the same exact
coefficient for $G_n$ and for $\overline{G_n}$. Intersect the two events by a
union bound; no further asymptotic loss is introduced.
\end{proof}
