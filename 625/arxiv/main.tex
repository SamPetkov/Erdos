...
\begin{abstract}
Let $\zeta(G)$ be the least number of parts in a partition of $V(G)$ into
cliques and independent sets.  For $G_n\sim G(n,1/2)$, we prove along the full
sequence of integers that
\[
 \mathbb P\!\left(\chi(G_n)-\zeta(G_n)\ge
 \frac{(\log 2)^2}{4}\log\!\left(\frac{200}{153}\right)
 \frac{n}{(\log n)^3}\right)\longrightarrow1.
\]
This resolves the question of Erd\H{o}s and Gimbel asking whether the
difference tends to infinity with high probability.  Writing $\delta_n$ for
the fractional part of the standard independence-number center, the
phase-resolved coefficient is $(\log 2)^2A_4(\delta_n)/4-o(1)$, where $A_4$ is
explicit, continuous, and nonconstant.  The proof must remain uniform when
the natural class-size cutoff jumps.  Its main tools are a signed profile on
four consecutive class sizes, an exact signed-overlap identity, a canonical
decomposition of large overlap cells, and a matching restriction for the
residual cycle-space sum.
A bounded-differences argument turns the resulting second-moment seed into a
...
written proof for counterexamples, revise the manuscript's exposition, audit
symbolic consistency, and generate candidate Lean code. Aristotle, Harmonic's
