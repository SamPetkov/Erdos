...
\author{%
  \textsc{Samuil Petkov}\\[0.35em]
  \small École normale supérieure--PSL (ENS--PSL)\\[-0.05em]
  \small\href{mailto:samuil.petkov@ens.psl.eu}{\texttt{samuil.petkov@ens.psl.eu}}%
}
...
The candidate argument has the following structure. Sections~II--IV establish
an unrestricted lower location for \(\chi(G_n)\). Section~V constructs a
four-size signed cocolouring profile whose first moment is positive at a
threshold lower by order \(n/(\ln n)^3\). Sections~VI--IX establish the
corresponding normalized second-moment estimate: Section~VI gives the exact
overlap and sign decomposition, Section~VII controls common whole classes,
Section~VIII sums the high-cell skeletons, and Section~IX bounds the
remaining conditional local and cycle contributions. Section~X amplifies the
resulting possibly rare cocolouring event to a high-probability statement.
Finally, Section~XI combines the chromatic lower location with the
cocolouring upper location. The roadmap records dependencies only; each
asymptotic estimate is proved at the point where it is used.
...
A \emph{signed cocolouring} is a partition in which every class is
...
witnesses for ordinary cocolourings rather than a new graph invariant.
...
The exact overlap representation of Section~VI still permits two partitions
...
partial-diagonal estimate is the input used in Section~VIII to control the
dense transportation sum. The counting identities in (7.1)--(7.4) are
finite-\(n\) identities; asymptotic estimates are explicitly labelled below.
...
\paragraph{Purpose of this section.}
Section~VII controls the diagonal contribution coming from common whole
...
unexposed residual configuration are deliberately deferred: Section~IX will
...
\(\mathcal M\) and no cell above \(R_0\). Multiplication of (8.3) by
...
high-skeleton estimate from Section~VIII.
...
\[
 \mathcal A(\mathcal M,j)=\mathbb E_{\mathrm{res}}\left[
...
Here \(\mathbb E_{\mathrm{res}}\) is only over the unexposed matching of
...
one row vertex. We obtain an upper bound by enlarging the family of simple
cycles to alternating walks: repeated vertices are permitted and the final
return-to-start constraint is omitted. The resulting walks still have at
least four residual edges, and successive row- and column-sum bounds by
\(\tau\) give the geometric series in (9.16):
...
For a cycle meeting \(\mathcal M\), distinguish and orient one matching
edge. There are at most \(2h\) choices. Cutting at every matching edge
leaves nonempty residual walk segments. The residual-walk kernel has
row-sum norm at most

\[
 b=\sum_{\ell\ge1}\tau^\ell=\frac\tau{1-\tau},          \tag{9.17}
\]

while traversal of a matching edge is a partial permutation of norm one.
After the initial distinguished matching edge there is no further factor
\(h\). Thus, if the relaxed cycle uses \(r\) matching edges, its total
remaining weight is at most \(b^r\). Therefore
...
colouring of \(G[V_{\mathrm{left}}]\) gives
...
\[
 \Pr\{\chi(G_n)-\zeta(G_n)\ge K\}\longrightarrow1.      \tag{11.3}
\]
