...
stronger conclusion is phase-resolved: if $\delta_n$ is the fractional part
of the usual independence-number center, an explicit function $A_4$ gives the
  coefficient $(\log 2)^2A_4(\delta_n)/4-o(1)$. A uniform estimate for $A_4$
then yields the numerical constant $0.032175871697936\ldots$. The main
difficulty is uniformity when the natural class-size cutoff changes. We use a
signed profile supported on four consecutive class sizes, an exact
signed-overlap identity, and a matching restriction for the residual
cycle-space sum. A bounded-differences argument amplifies the resulting
second-moment witness to a high-probability cocoloring.
\end{abstract}
...
$\binom nk2^{-\binom k2}$.  Its first-moment threshold is centered at the
following real approximation; we also record its fractional phase:
\[
  \alpha_0(n)
  :=2\log_2 n-2\log_2\log_2 n+2\log_2(\mathrm e/2)+1,
  \qquad
  \delta_n:=\alpha_0(n)-\lfloor\alpha_0(n)\rfloor.
\]

\begin{maintheorem}[Phase-resolved and uniform forms]
There is an explicit function $A_4:[0,1]\to(0,\infty)$, defined in
\eqref{eq:phase-function-definition-v3}, and a nonnegative deterministic
sequence $\varepsilon_n\to0$ such that, for $G_n\sim G(n,1/2)$,
\[
  \mathbb P\!\left(
    \chi(G_n)-\zeta(G_n)
    \ge
  ...
  \longrightarrow1.
\]
Moreover, $A_4$ is continuous and
\[
...
\]
The first assertion is stronger: it retains the actual phase
$A_4(\delta_n)$ rather than replacing it by a phase-independent lower bound.
...
by an exact overlap identity. The same four-size support covers every phase; exact
endpoint-table summation and residual cycle-space restriction make its second
moment uniform.

\subsection*{Contributions and main ideas}

Beyond resolving the problem, the argument provides five ingredients.  The
root and partial-diagonal estimates are tailored to the four-size profile;
the overlap identities, matching comparison, and amplification principle are
stated in reusable finite or quantitative forms.

\begin{enumerate}
\item \textbf{Phase-resolved root separation.} For the ordinary first-moment
root $r_+$ and the signed four-size root $r_4^{\mathrm{co}}$, we prove,
uniformly over the complete phase,
\[
  r_+-r_4^{\mathrm{co}}
...
residual matching with a no-return condition.  Thus no sign correlation is
estimated before its finite combinatorial structure has been identified.

\item \textbf{Uniform control of all common subprofiles.} For the exposed
diagonal weights $D(\ell)$, we prove
\[
...
reverse recurrence.  This is the direct four-size replacement for the
tame-profile estimates whose hypotheses do not cover the exceptional phase.
...
\item \textbf{Residual restriction and quantitative amplification.} After the
high cells are exposed, restriction outside their matching is injective on
even residual edge sets.  This gives a product bound for the remaining local
rewards and cycle-space factor, and hence
\[
  \frac{\mathbb E Z^2}{(\mathbb E Z)^2}
  \le
  \exp\!\left\{o\!\left(\frac{n}{(\log n)^4}\right)\right\}.
\]
Paley--Zygmund supplies a possibly rare signed witness.  More generally, if a
$k_n$-class cocoloring has probability at least $e^{-\Lambda_n}$, then for
every deterministic $r=r(n)>0$ the amplification lemma adds at most
\[
  C\!\left(
...
\[
  ...
  \log\!\left(\frac{200}{153}\right).
\]
Placing the signed witness a fixed distance above its first-moment root retains
the full leading root-separation coefficient.  Since $A_4$ is continuous
and its lower certificate is strict on the compact phase interval, the same
factor is retained in the uniform consequence.  Thus the finite support is not
merely a technical truncation: it is the device that converts phase dependence
into a full-sequence theorem.
...
Sections~1--5 prove (A) and (B) by locating the ordinary and signed
first-moment roots, constructing an exactly feasible four-size profile, and
controlling every phase uniformly.  For (C), Section~6 evaluates the signed
overlap law exactly, and Section~7 controls all common subprofiles.
Sections~8 and~9 decompose every overlap uniquely into its canonical high
cells and its capped residual part.  The former estimate contains only the
exposed high-cell factors, while the latter contains the remaining local
rewards and binary cycle-space factor.  Together they give a normalized
...
\[
  ...
  =\exp\!\left\{o\!\left(\frac{n}{(\log n)^4}\right)\right\}.
\]
Section~10 amplifies the resulting seed, and the final section combines
(A)--(C) and extracts the explicit constants.
...
\[
  ...
  \qquad C:=1+\log q-q.
\]
The \emph{independence-number center} is the standard real approximation to
the value of $s$ at which the expected number $\mu_s$ of independent $s$-sets
is one. Its fractional part records the phase of the neighboring integer
sizes. Without changing logarithm base, it is
\begin{equation*}
  \alpha_0
  =2\log_2 n-2\log_2\log_2 n+2\log_2(\mathrm e/2)+1
  =\frac{2(L-\ell+C)}q+1.
...
Lemma~3.1, write $L_+(n,k):=L_{S_+}(n,k)$, and put
\begin{equation*}
...
coordinates and $|R(k_i)|=O(\!\log(n+1))$. Maximizing over the feasible profile
and then summing (4.3) yields
\begin{equation*}
  \log E_{n,k,\alpha+1}
  \le L_+(n,k)+O((\log n)^2).
  \tag{4.3a}
...
\begin{equation*}
  ...
  \tag{4.4}
\end{equation*}
Combining (4.3a) and (4.4), and decreasing the constant once, gives a
...
\(\lambda_S(T)\) with \(M_S(\lambda_S(T))=T\). The optimizer
...
\[
 ...
 =\frac{b+1/4+b/16}{1+b+1/4+b/16}<51/100.
\]
...
\pagebreak[4]
For the sole purpose of placing a fixed-offset integer strictly between the
two roots, we record the following deliberately coarse consequence: for all
sufficiently large \(n\),
...
Consequently the common corridor in Lemma~3.1 contains the entire interval
...
rounded.  The value $16$ is not optimized; any fixed positive integer suffices
asymptotically, while this choice leaves a generous explicit margin in (5.19).

We call the resulting profile the \emph{exact signed witness profile}.  Its
total number of parts is a fixed distance above the signed four-size root, and
the type counts are chosen next so that both finite-\(n\) conservation
...
We also record why the whole rounding segment remains in the positive
orthant. By (5.15), $k_i^*\ge c_pk_{\mathrm{co}}$, and hence, uniformly in
...
profile-multiplicity factorials \(k_i!\), the exact signed first moment is
...
\[
  ...
  \ge16c_1(\log n)^2.
\]
The $O(\log n)$ Stirling error in (5.19) is smaller than this bound; decreasing
$c_Z$ proves the assertion. Every
...
For orientation, the remaining argument uses the scale hierarchy
\[
  k_{\mathrm{co}}\asymp\frac n{\log n},\qquad
  r_+-r_4^{\mathrm{co}}\asymp\frac n{(\log n)^3},\qquad
  \Lambda_n=o\!\left(\frac n{(\log n)^4}\right).
\]
The last estimate is the second-moment target proved in Sections~8--9. Once it
is available, its amplification cost satisfies
\[
...
component; the colors and line styles distinguish components, not sign values.
The displayed arithmetic verifies (6.4) for this example.}
...
The quantity $D(\ell)$ is the aggregate exposed incidence and local signed
reward of the marked whole blocks. It is not the complete second-moment
contribution conditional on those blocks: all unexposed local rewards and the
residual cycle-space factor remain to be accounted for in Sections~8--9.
...
Whenever \(\ell_i<k_i\), one may add a common block of type \(i\); first note
that
...
For reference, the proof assigns one tool and one target estimate to each
range:
\begin{center}
\small
...
\hline
Selected-mass range & Controlling device & Contribution to (7.7) \\
\hline
$m\le\eta n$ & forward recurrence (7.4) & $1+o(1)$ \\
$m>\eta n$, $n-m>n/32$ & rate function $\Phi_T$ & $o(1)$ \\
$n-m\le n/32$ & reverse recurrence (7.6) & $o(1)$ \\
\hline
\end{tabular}
\end{center}
...
Here is the complete factorial cancellation. Put $K=k_{\mathrm{co}}$, so
$k_i=Kp_i$, $\ell_i=Ky_i$, and $k_i-\ell_i=Kz_i$. The factorials involving
the profile multiplicities contribute
\[
\begin{split}
...
\begin{equation*}
 ...
 \tag{7.19}
\end{equation*}
Combining (7.14), (7.15a), (7.18), and (7.19) yields one absolute constant
$C$ such that
\begin{equation*}
 \log D(\ell)
...
\begin{equation*}
  ...
  \tag{7.23}
\end{equation*}
All rational inequalities in this two-range argument are explicit; the
...
or cycle-space factor is included here; those factors belong to Section~9.
...
Proposition~\ref{prop:canonical-exact-overlap-decomposition-v3} supplies its
labeled-matching lift, while the last line separates the two factors that are
bounded independently. In the figure, \(w_{\mathrm{hi}}\) denotes the bare
skeleton weight defined below, and \(\mathcal A\) denotes the residual
attachment defined in Section~9; these are roadmap labels at this point.
...
\begin{tikzpicture}[
  ...
  \node[font=\small] at (9.75,-3.10) {$\displaystyle
    ...
    =\sum_{(P,j)}
      \underbrace{w_{\mathrm{hi}}(P,j)}_{\text{bare skeleton}}
      \qquad
...
\(e\in P\); no other stub pair is prescribed.  Thus
...
For a fixed matching support $P$ and a multiplicity vector
$j=(j_e)_{e\in P}$ satisfying
$R_0<j_e\le\min\{s_e,t_e\}$ for every $e\in P$, the sum of the bare weights over
\(\mathfrak F(P,j)\) is
\begin{equation}
  w(P,j)
  =
...
\[
  ...
  \qquad 0\le i\le3.
\]
Here the subscript $i+2$ is a deficit label, not a class size:
...
condition $\rho_{\Sigma}\le1$ and cleanly separates the deficit sum from endpoint
tables.
\end{remark}
...
included in the common eventuality threshold of Section~10.1.
...
Assume $\alpha>8$. Then every feasible endpoint table $L$ satisfies
\begin{equation}
  W(L)^2
  \le
...
most $(1+3\tau_n^{\mathrm{end}})^K\sum_v D(v)$, where $v$ is now a dummy
subprofile variable. The second half is identical after exchanging rows and
columns. Section~7 proves $\sum_v D(v)=1+o(1)$ uniformly in the phase.
...
Let $\mathfrak S_n$ denote the bare high-skeleton sum:
\[
  \mathfrak S_n
  :=
...
$\mathfrak S_n$. Section~9 treats exactly those remaining
...
In this section the canonical support denoted by $P$ in Section~8 is written
$M$. This is a notational renaming, not an additional matching.
...
\(S\in\mathfrak F(M,j)\) is an exactly-\(J\)-pair physical partial matching.
...
By itself, $w_{\mathrm{hi}}(M,j)$ is the aggregate weight of the partial
matchings in $\mathfrak F(M,j)$; it is not the probability of the event that
these and only these cells have the prescribed full multiplicities. The
indicator $\mathbf 1_{\mathcal E(M,j)}$ in the residual expectation supplies
the missing cap and no-return conditions.
...
\]
The degree cap makes $M(\Pi)$ a matching. Moreover,
...
Proposition~\ref{prop:aggregate-high-weight-v3}. The unexposed local rewards
and the remaining sign factor are exactly the integrand defining
...
$x_a=\sum_bx_{ab}$, $x'_b=\sum_ax_{ab}$, and $X=\sum_{a,b}x_{ab}$. If
$X\le m_0$, Markov's inequality applied to the product of binomial cell counts
gives
\[
...
\]
In this range, $(m_0)_X\ge(m_0/\mathrm e)^X$; if $X>m_0$, the event is empty.
Hence, in all cases,
\begin{equation}
  \mathbb P_{\mathrm{res}}(r'_{ab}\ge x_{ab}\text{ for all }a,b)
  \le
...
This reward expansion is exact only on $\mathcal E(M,j)$, where every residual
multiplicity is at most $R$. After multiplication by
$\mathbf 1_{\mathcal E(M,j)}$, we may remove the indicator to obtain an upper
bound for the nonnegative truncated expansion; no identity with the uncapped
reward is used outside $\mathcal E(M,j)$.
...
For completeness, choose an integer $U_0$ beyond which the preceding two
...
bound. We now record their error terms in a form that can be used directly by
the amplification theorem and by the formal development.
...
\begin{equation}
  \sum_v D(v)\le1+\varepsilon_n^{\mathrm{pd}}.
...
$\mathfrak S_n$ defined in Section~8. For all sufficiently
large $n$,
\begin{equation}
  \mathfrak S_n
...
The same $C$ and $\varepsilon_n$, independent of $k_n,\Lambda_n,r$, have the
following property: for every deterministic choice $r=r(n)>0$, the bound
...
The final assembly uses the preceding estimates simultaneously over every
admissible phase, integer profile, skeleton, and residual degree sequence. The
following table records where that uniformity enters.
\begin{center}
\small
\renewcommand{\arraystretch}{1.16}
\begin{tabular}{@{}p{0.22\linewidth}p{0.69\linewidth}@{}}
\hline
Input & Uniform data passed to later sections \\
\hline
Phase and root package &
$\delta\in[0,1]$ and $T\in K_*$; the inverse mean maps have
$M'_{n,S}\ge v_*>0$, and $\omega_n^{\mathrm{root}}\to0$ is independent of
the phase. \\
Signed witness profile &
$A_4(\delta)>\gamma_4$, $p_i\ge c_p$,
$\log\Exp{Z_{\mathbf k}^{\mathrm{sgn}}}\ge c_Z(\log n)^2$, and
$\|\Delta^{\mathrm{tot}}\|_\infty\le C_\Delta$, with the same constants for
all phases. \\
Partial diagonals &
The three disjoint mass ranges in the Section~7 roadmap cover every
$0\le\ell_i\le k_i$, and $\varepsilon_n^{\mathrm{pd}}\to0$ uniformly. \\
High skeletons &
After one common threshold,
$\alpha>8$, $U\ge2$, $\alpha\ge(5/2)\log n$, $\rho_{\Sigma}\le1$, and
$\tau_n^{\mathrm{end}}\le1$; hence
$\Gamma_n^{\mathrm{skel}}=o(n/(\log n)^4)$ uniformly over feasible skeletons.
\\
Residual attachments &
The cases $m_0=0$, $2^U\le m_0^3$, and $2^U>m_0^3$ are exhaustive, and one
$\Gamma_n^{\mathrm{att}}=o(n/(\log n)^4)$ works for every residual degree
sequence and skeleton. \\
Amplification &
The event $\mathcal E_n$ controls every leftover vertex set simultaneously;
the constants and $\varepsilon_n$ in Lemma~10.2 are independent of
$k_n$, $\Lambda_n$, and $r$. \\
\hline
\end{tabular}
\end{center}
Only finitely many eventuality thresholds occur in this table. Taking their
maximum gives one deterministic, phase-independent threshold for the final
argument.
...
$k_{\mathrm{co}}=\lceil r_4^{\mathrm{co}}\rceil+16$ is the witness count, and
$k_{\mathrm{co}}+a_n$ and $k_\chi^-$ are probabilistic thresholds.
...
\]
tends to zero.  On the intersection of the two high-probability events above,
\[
  \chi(G_n)-\zeta(G_n)
  > k_\chi^- - k_{\mathrm{co}}-a_n
  \ge
...
A companion Lean~4 development formalizes a conservative explicit consequence
of the main theorem; its top-level
declaration is \texttt{Erdos625.erdos625}, with type
\texttt{Erdos625.Erdos625Statement}~\citep{petkov-erdos625-lean}.
The formal statement uses the coefficient
$((\log 2)^2/32)\log(200/153)$ and asserts convergence along the full sequence
of natural numbers. Thus the Lean kernel checks a conservative explicit
consequence that already resolves Erd\H{o}s Problem~625 at that constant; the
fixed-offset construction, the phase-resolved refinement, and the stronger coefficient
$((\log 2)^2/4)\log(200/153)$ are established in the manuscript proof. The cited exact Git
source revision pins Lean and Mathlib at version~4.31.0 and provides the
modular sources and deterministically generated single-file closure; the cited
archive revision adds the trust-boundary audit and clean-environment replay
record. Both revisions provide detailed build instructions.
Detailed reproduction commands, checksums, and transcripts are maintained in
the companion documentation.

\paragraph{AI assistance disclosure.}
OpenAI GPT-5.6 Sol, used through ChatGPT and Codex, assisted with adversarial
and counterexample-oriented checking of the written proof, formalization
design and review, and development of Python scripts for separate numerical
and symbolic consistency checks. Aristotle, Harmonic's automated theorem-proving system
\citep{achim-et-al-2025}, assisted with the Lean formalization through
quarantined atomic proof search and clean cloud replay. Candidate formal
outputs were reviewed, integrated, and rebuilt under the pinned Lean toolchain
before acceptance. Neither system is an author, and no AI output was treated
as proof of the manuscript theorem: machine-checked claims rest on the Lean
kernel, while unformalized claims rest on the written argument. Samuil Petkov
directed the work, selected the claims and arguments, reviewed the manuscript
and formalization, and takes responsibility for the accuracy, integrity,
citations, and final submission.
