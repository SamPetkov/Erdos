...
\begin{tikzpicture}[
  ...
  vertex/.style={circle,draw=black!70,fill=white,inner sep=1.8pt},
  cycleedge/.style={draw=blue!60!black,line width=0.9pt},
  treeedge/.style={draw=orange!80!black,line width=0.9pt,dashed}
]
  \begin{scope}
    \fill[blue!10] (0,2) rectangle (2,3);
    \fill[blue!10] (0,1) rectangle (2,2);
    \fill[orange!16] (2,0) rectangle (3,1);
    \draw[gray!65] (0,0) grid (3,3);
    \foreach \x/\y/\z in {
      0.5/2.5/2,1.5/2.5/2,2.5/2.5/0,
      0.5/1.5/2,1.5/1.5/3,2.5/1.5/0,
      0.5/0.5/0,1.5/0.5/0,2.5/0.5/2}{
      \node at (\x,\y) {$\z$};
    }
    \foreach \y/\a in {2.5/1,1.5/2,0.5/3}{
      \node[left] at (-0.05,\y) {$a_{\a}$};
    }
    \foreach \x/\b in {0.5/1,1.5/2,2.5/3}{
      \node[above] at (\x,3.02) {$b_{\b}$};
    }
    \node[font=\footnotesize] at (1.5,3.75) {overlap table $r$};
  \end{scope}

  \draw[->,line width=0.65pt] (3.55,1.5) -- (6.0,1.5)
    node[midway,above,align=center] {$r_{ab}\ge2$};

  \begin{scope}[shift={(6.6,0)}]
    \foreach \y/\a in {2.5/1,1.5/2,0.5/3}{
      \node[vertex] (a\a) at (0,\y) {$a_{\a}$};
    }
    \foreach \y/\b in {2.5/1,1.5/2,0.5/3}{
      \node[vertex] (b\b) at (4,\y) {$b_{\b}$};
    }
    \draw[cycleedge] (a1)--(b1) (a1)--(b2) (a2)--(b1) (a2)--(b2);
    \draw[treeedge] (a3)--(b3);
    \node[font=\footnotesize] at (2,3.75) {support graph $H(r)$};
    \node[align=center] at (2,-0.20)
      {$|E|=5,\ |V|=6,\ c=2,\ \beta=1$};
  \end{scope}

  \draw[->,line width=0.65pt] (11.25,1.5) -- (13.0,1.5);
  \node[align=left,anchor=west,font=\footnotesize] at (13.35,1.55)
    {$\displaystyle W=7,\qquad \text{local reward}=4,$\\[0.45ex]
     $\displaystyle \text{normalized factor}
       =2^{7+2-6}=8=4\cdot2^1.$};
\end{tikzpicture}
\caption{From an overlap table to the signed support factor. The shaded cells
