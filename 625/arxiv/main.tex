...
The exact decomposition used across Sections~8--9 is summarized in
Figure~\ref{fig:canonical-overlap-decomposition-v3}. Its matrix panels record
the finite extraction proved in Proposition~\ref{prop:canonical-exact-overlap-decomposition-v3},
while the last line separates the two factors that are bounded independently.
...
\begin{tikzpicture}[
  ...
  \node[font=\large] at (6.0,2) {$\longmapsto$};
  ...
  \begin{scope}[shift={(7,0)}]
    ...
    \node[font=\footnotesize] at (2.5,4.65)
      {$P=\{(a,b):r_{ab}>R_0\}$};
    \node[align=center,text width=3.4cm] at (2.5,-0.85)
      {high matching $P$ and values $j=r|_P$};
  \end{scope}

  \node[font=\large] at (13.25,2) {$\sqcup$};
...
\end{tikzpicture}
\caption{Canonical high-cell extraction, shown on a schematic overlap table.
Highlighted cells lie in distinct rows and columns and therefore form a
matching. Proposition~\ref{prop:canonical-exact-overlap-decomposition-v3}
proves the finite bijection and the displayed normalized factorization.}
...
most $(1+C\tau_n^{\mathrm{end}})^K\sum_v D(v)$, where $v$ is now a dummy
...
$\alpha>8$, $U\ge2$, $\alpha\ge(5/2)\log n$, $\rho_{16}\le1$, and
...
The blue interval is the usable separation;
$a_n=o(n/(\log n)^3)$ and
...
An accompanying Lean~4 source formalization defines the finite uniform
$G(n,1/2)$ model and contains the theorem
\texttt{Erdos625.erdos625 : Erdos625Statement}. Its statement uses the
displayed constant $(\log 2)^2\log(200/153)/32$ and quantifies along the full
sequence of natural numbers. The modular project and its deterministic
single-file closure are archived at the immutable repository commit
\url{https://github.com/SamPetkov/Erdos/tree/86c4422f9b41c4ce50e7c920dc7349a9f07f24a8/625/formalization}
with Lean and Mathlib pinned to \texttt{v4.31.0}. The generated file contains
475 local modules, 86 external imports, and 88,563 lines; its SHA-256
is \url{0f54ac26285e5a1631ed62f0258ba0836cc3a2cd28cc35df3b2f737cfaffac30}.
The deterministic generator check establishes source closure. On 27 August
2026, that exact file was also replayed in a clean cloud environment with the
pinned toolchain by running
\texttt{lake env lean -DwarningAsError=true Erdos625SelfContained.lean}; Lean
returned exit code zero. The final
\texttt{\#print axioms Erdos625.erdos625} output lists exactly
\texttt{propext}, \texttt{Classical.choice}, and \texttt{Quot.sound}. The
complete merged standard-output and standard-error transcript, exit code,
source and toolchain revisions, file checksums, and audit report are preserved
at the immutable public replay commit
\url{https://github.com/SamPetkov/Erdos/tree/1eb0aeba305327d383496b912a1e6e1a8fa71947/625/formalization/replay/aristotle-76792321-4b5f-4367-8885-d1470db80091}.
