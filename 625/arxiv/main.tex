...
$i\mapsto d=i+2$ from $\{0,1,2,3\}$ to $\{2,3,4,5\}$, with inverse
$d\mapsto d-2$. Define
\[
  \kappa_i:=k_{i+2},
  \qquad
...
For every finite $n$ and every fixed signed profile,
\begin{equation}
...
\]
The degree cap makes $M(\Pi)$ a matching. Let $S(\Pi)$ be the set of all stub
pairs of $\Pi$ lying in cells of $M(\Pi)$, and put
...
$\alpha\ge(5/2)\log n$, $\rho_{16}\le1$, and
