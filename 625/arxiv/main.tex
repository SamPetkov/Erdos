...
The exact decomposition used across Sections~8--9 is summarized by the
following diagram. The first arrow is a finite bijective extraction, while the
second line separates the two factors that are bounded independently:
\[
\begin{gathered}
  \text{full overlap matching }\Pi
  \quad\longmapsto\quad
  \left(
    \underbrace{M,j,S}_{\text{canonical exposed high cells}},
    \underbrace{\Pi_{\mathrm{res}}}_{\text{capped residual matching}}
  \right)
  \\
  \Big\downarrow\ \text{sum the physical high-cell fiber and condition on the residual law}
  \\
  \underbrace{w_{\mathrm{hi}}(M,j)}_{\text{bare skeleton}}
  \quad\times\quad
  \underbrace{\mathcal A(M,j)}_{\text{residual attachment}}.
\end{gathered}
\]
