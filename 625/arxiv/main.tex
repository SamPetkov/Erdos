...
At the level of dependencies, the proof has the following shape:
\[
\begin{gathered}
\text{phase-uniform roots}
\longrightarrow
\text{tangent-rounded signed midpoint}
\longrightarrow
\text{exact signed-overlap law}
\\[-1pt]
\Big\downarrow
\\[-1pt]
\text{partial diagonals}
\;+\;
\text{canonical high skeleton}
\;+\;
\text{residual attachments}
\\[-1pt]
\Big\downarrow
\\[-1pt]
\text{rare signed witness}
\longrightarrow
\text{amplification}
\longrightarrow
\text{full-sequence gap}.
\end{gathered}
\]
...
signed profile. The partial-diagonal weights are handled first and then used
as a reference measure for the endpoint-table sum; no overlap is deleted at
this stage. Each overlap table is subsequently decomposed into canonical high
cells and residual cells. Sections~8 and~9 keep these two accounts disjoint:
no residual factor is paid in the high-cell estimate, and no high-cell
multiplicity is paid again in the residual estimate. The logarithm of the
final upper bound is explicitly
\[
  \Lambda_n
...
Section~8 locally reindexes the four deficit labels by $i\mapsto i+2$ and
defines the associated endpoint sizes, block counts, and endpoint tables. Its
deficit-indexed counts are distinct from the size-indexed convention $k_s$
above; Section~9 inherits the Section~8 convention unchanged.
...
By (5.12) and the common corridor in Lemma~3.1, this integer is eventually
feasible for \(S_4\), and its target deficit mean lies in the fixed compact
interval \(K_*\subset(2,5)\).
...
Here is the complete cancellation ledger. Put $K=k_{\mathrm{co}}$, so
...
Thus the endpoint-table estimate pays only the bare-skeleton factor, and the
matching-restriction estimate pays only the residual attachment.
...
\]
takes values in the feasible skeletons, their physical fibers, and residual
...
\subsection{Uniformity dependency ledger}
...
following ledger records where that uniformity enters.
\begin{center}
\small
...
the phase. \\
Midpoint profile &
$A_4(\delta)>\gamma_4$, $p_i\ge c_p$, and
...
\end{tabular}
\end{center}
Only finitely many eventuality thresholds occur in this ledger. Taking their
...
\begin{tikzpicture}[
  ...
  \draw[blue!55!black,line width=1.6pt] (6.5,0) -- (10.6,0);

  \foreach \x in {0,5.4,6.5,10.6,11.5}
    \draw[line width=0.55pt] (\x,-0.10) -- (\x,0.10);

  \node[below=3pt] at (0,0) {$r_4^{\mathrm{co}}$};
  \node[above=3pt] at (5.4,0) {$k_{\mathrm{co}}$};
  \node[below=3pt] at (6.5,0) {$k_{\mathrm{co}}+a_n$};
  \node[above=3pt] at (10.6,0) {$k_\chi^-$};
  \node[below=3pt] at (11.5,0) {$r_+$};
  \node[text=blue!55!black,fill=white,inner sep=1.5pt] at (8.55,0.42)
    {usable separation};
  ...
  \draw[<->] (5.4,-1.30) -- (11.5,-1.30);
  \node[below=2pt,font=\scriptsize] at (8.45,-1.30) {$\displaystyle
    r_+-k_{\mathrm{co}}
    =\left[\frac{(\log 2)^2}{8}A_4(\delta_n)+o(1)\right]
      \frac{n}{(\log n)^3}$};
\end{tikzpicture}
...
$k_{\mathrm{co}}$ is the rounded witness count, and
...
A companion Lean~4 development formalizes the main theorem; its top-level
declaration is \texttt{Erdos625.erdos625}, with type
\texttt{Erdos625.Erdos625Statement}~\citep{petkov-erdos625-lean}.
The formal statement uses the same explicit constant as above and asserts
convergence along the full sequence of natural numbers. The cited exact Git
