...
difference tends to infinity with high probability.  More precisely, if
$\delta_n$ is the fractional part of the standard independence-number center,
then our lower bound has coefficient $(\log 2)^2A_4(\delta_n)/4-o(1)$, where
$A_4$ is explicit, continuous, and nonconstant.  Uniformity at the jumps of
the natural class-size cutoff is
the central difficulty.  We use a signed profile on four consecutive class
sizes, an exact signed-overlap identity, a canonical decomposition of large
overlap cells, and a matching restriction for the residual cycle-space sum.
...
The proof has the following components. Three are stated in forms not specific
to Problem~625: the finite signed-overlap identity, the fixed-alphabet matching
comparison, and the quantitative seed-amplification lemma.
...
The phase dependence is genuine. Put $\lambda_*=3q$, define
...
This section estimates the large overlap cells.  We first sum all labeled stub
...
\]
This is the same high-skeleton weight that appears in the exact
...
$u_i=\alpha-(i+2)$ on each partition side. This is the deficit-indexed
convention of Section~5, distinct from the size-indexed notation $k_s$ used
earlier. For
...
This reference-sum domination preserves every common-class overlap. Finally,
$K=\Theta(n/\log n)$ and
...
with the residual binary cycle-space factor. This estimate is applied termwise
to the exact overlap sum; no overlap matrix is discarded.
...
\eqref{eq:exact-attachment-decomposition-v3}, with neither omissions nor
double counting.
\end{proof}
...
This identity uses both parts of $\mathcal E(M,j)$: the cap bounds every
off-$M$ multiplicity by $R$, and the no-return condition makes $r'$ vanish on
$M$. After multiplying by $\mathbf 1_{\mathcal E(M,j)}$, we remove this
...
We now combine the zero-residual case with the two positive-residual regimes
...
\end{equation}
This inequality displays separately the ambient deficit contribution, the
off-diagonal endpoint-table contribution, and the partial-diagonal reference
mass.
...
\[
  ...
  =e^{\Lambda_n}.
\]
The lower bound is the nonnegativity-of-variance inequality. Equations
...
cycle-space factor. This is the factorization used in the amplification step.
...
location, and the amplification estimate.
