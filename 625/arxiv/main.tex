...
Section~8 uses the bijection $i\mapsto i+2$ from $\{0,1,2,3\}$ to the four
deficit labels $\{2,3,4,5\}$. The corresponding endpoint sizes and available
block counts are defined there from the deficit-indexed four-size profile; this
is distinct from the size-indexed convention $k_s$ above. For a block support $P$, the
...
\[
  ...
  \qquad 0\le i\le3.
\]
Thus $\kappa_i$ is the number of endpoint blocks of size $u_i$ on each of the
two partition sides. For
...
For $L=(\ell_{ij})\in\mathbb N_0^{4\times4}$, write
\[
  r_i=\sum_j\ell_{ij},
  \qquad
  c_j=\sum_i\ell_{ij}.
