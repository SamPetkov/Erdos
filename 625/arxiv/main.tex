...
In the shifted four-size indexing used in Section~8, the endpoint sizes are
$u_i=\alpha-2-i$ for $i\in\{0,1,2,3\}$, and
$\kappa_i:=k_{i+2}$ is the number of available blocks of type $i$. For a block
support $P$, the endpoint table $L(P)=(L_{ij})$ records how many support edges
join types $i$ and $j$. A table is \emph{feasible} when its type-$i$ row sum
and type-$j$ column sum are at most $\kappa_i$ and $\kappa_j$, respectively.
Every block support has a feasible table, and every feasible table is attained
by a block-slot matching.
...
\item
  ...
  \(0\log 0=0\), the last two terms may be replaced by
  \(O(\log(m+1))\).
\item
...
\end{split}
  \tag{2.7}
\end{equation*}
This function is continuous, hence bounded, on $[0,1]$. Choosing
$C_{\mathrm{ph}}$ sufficiently large absorbs all displayed remainders into
$\varepsilon_n^{\mathrm{ph}}$.
...
measure for the endpoint transport below. Every overlap matrix remains present
...
matchings exposing these prescribed pairs is
\begin{equation}
  w(P,j)
  =
  \frac{
...
For the remainder of this section, reindex the profile from the deficit labels
$2,3,4,5$ used in Sections~5--7 by
\[
  \kappa_i:=k_{i+2},
  \qquad
  u_i:=\alpha-2-i,
...
Recall the shifted notation $u_i=a-i$, $\kappa_i=k_{i+2}$ for
$0\le i\le3$, where $a=\alpha-2$.
...
\end{equation}
Under the reindexing $r_i=\ell_{i+2}$, this is exactly the partial-diagonal
weight from Section~7. Every sum over $r$ below is over this finite coordinate
box; no value outside it is used.
...
\end{equation}
We first bound the integrand pointwise; after that bound is established, the
indicator of every residual restriction may be replaced by one. Since each
residual cell count is at most $U$,
\[
  \prod_{a,b}g(r'_{ab})
  \le
...
event while adding fewer parts than the root separation can absorb. The
...
current set is replaced by its \(H\)-neighborhood, so every later choice is
adjacent in \(H\) to every earlier one. Starting from any
\(\lceil n^{1/3}\rceil\)-subset, (10.3a) stays above \(n^{1/4}\) for at
least \(\lfloor {\log n}/(13\log4)\rfloor\) steps. Thus the procedure constructs
...
divides it by two. The ceiling contributes $O(1)$; the tangent correction
