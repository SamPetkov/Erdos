...
$2^U\le m_0^3$ by the critical scale $U^2 2^{U/4}$.  Together they give an
explicit seed exponent of order $\sqrt n(\log n)^{5/2}$.
...
\begin{equation*}
  ...
  \begin{cases}
    ...
    \displaystyle
    \exp\!\left(\frac{\log 2}{2}U^3 2^{U/4}\right),&m_0<T_U.
  \end{cases}
  \tag{9.53}
\end{equation*}
...
\begin{aligned}
  ...
  \le{}&
  \log_2R+\binom R2-1
  +(R-2)\left(\log_2\mathrm e-\frac U4\right)
  -\log_2(R!).
\end{aligned}
\]
Because $R\le U/2$,
\[
  \binom R2-\frac U4(R-2)\le\frac U2.
\]
Moreover, $R\ge(U-1)/2$ and
$\log_2(R!)\ge(R/2)\log_2(R/2)$ for $R\ge4$.  Hence the displayed logarithm
tends to $-\infty$, and $Ra_R\le\theta^2$ for all sufficiently large $U$.
The finitely many remaining values are absorbed into one absolute constant.
...
\begin{equation*}
  ...
  \le
  \exp\!\left(C_\sharp\sqrt n\,(\log n)^{5/2}\right).
  \tag{9.56}
\end{equation*}
Consequently
\begin{equation*}
...
the two explicit cases $m_0\ge U^2 2^{U/4}$ and
$m_0<U^2 2^{U/4}$.  Neither interface requires
